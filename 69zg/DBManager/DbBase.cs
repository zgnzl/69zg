...
namespace _69zg.DBManager
{
    ...
    public class DbBase<T> : MSSQLManager where T : class, new()
    {

        public static new EntitiesSqlConnStr MasterDb = GetDmodel();
        ...
        {            
            //将 对象 添加到 EF中
            MasterDb.Set<T>().Attach(model); var setEntry = ((IObjectContextAdapter)MasterDb).ObjectContext.ObjectStateManager.GetObjectStateEntry(model);            //指定列修改
            if (isProUpdate)
            {
        ...
        {
            return null;// ordering == null ? SlaveDb.Set<T>().ToList(): SlaveDb.Set<T>().OrderBy(ordering).ToList();
        }       
        /// <summary>
         /// 带条件查询获取数据        
         /// </summary>
        ...
        {
            return null;// SlaveDb.Set<T>().Where(whereLambda).Select<TDto>().ToList();
        }        
